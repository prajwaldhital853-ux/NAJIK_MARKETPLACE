...
import { Eye, EyeOff, Lock, LogIn, Mail, ShieldAlert, User } from "lucide-react";
import { ApiError } from "@/lib/api";
import { firstAllowedPath } from "@/lib/rbac";
import { fetchStaffLockoutStatus } from "@/lib/staff-api";
...
const LOCKOUT_STORAGE_KEY = "najik_admin_login_lockout";

type StoredLockout = { email: string; lockedUntil: string };

function readStoredLockout(email: string): StoredLockout | null {
  try {
    const raw = sessionStorage.getItem(LOCKOUT_STORAGE_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as StoredLockout;
    if (parsed.email !== email.trim().toLowerCase()) return null;
    if (!parsed.lockedUntil || Date.parse(parsed.lockedUntil) <= Date.now()) {
...
function writeStoredLockout(email: string, lockedUntil: string) {
  sessionStorage.setItem(
    LOCKOUT_STORAGE_KEY,
    JSON.stringify({ email: email.trim().toLowerCase(), lockedUntil }),
  );
}
...
export default function StaffLoginPage() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const left = Math.max(0, Math.ceil((lockoutUntilMs - Date.now()) / 1000));
  ...
  useEffect(() => {
    ...
    const stored = readStoredLockout(email);
    if (stored) {
      setLockoutUntilMs(Date.parse(stored.lockedUntil));
    }
    ...
    const id = window.setTimeout(() => {
      void fetchStaffLockoutStatus(trimmed)
        .then((status) => {
          if (status.locked && status.lockedUntil) {
            applyLockout(status.lockedUntil, trimmed);
            if (status.detail) setError(status.detail);
            return;
          }
          if (!readStoredLockout(trimmed)) {
            setLockoutUntilMs(null);
          }
        })
        .catch(() => {});
  ...
  }, [email, verifyStep, applyLockout]);

  function formatLoginError(err: unknown) {
    if (err instanceof ApiError) {
      if (err.code === "account_locked" || err.lockout) return err.message;
      if (err.message.toLowerCase().includes("account locked")) return err.message;
      if (err.message.includes("attempts remaining")) return err.message;
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (err instanceof ApiError && err.lockout) {
        applyLockout(err.lockout.lockedUntil, attemptEmail);
      }
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-[12px] leading-relaxed text-[#b45309]">
                              Too many failed login attempts on this device. You can try again in{" "}
