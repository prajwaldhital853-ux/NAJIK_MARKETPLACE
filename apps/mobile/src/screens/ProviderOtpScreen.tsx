...
import { clearProviderRegisterDraft, peekProviderRegisterDraft } from "../providerRegisterDraft";
import { colors, shadow } from "../theme";

const GREEN = "#1B7D2C";

export function ProviderOtpScreen() {
  const insets = useSafeAreaInsets();
  const navigation = useNavigation<any>();
  const { completeProviderSignup } = useAuth();
  ...
  async function submit() {
    ...
    setVerifying(true);
    try {
      await completeProviderSignup(code.trim() || "1234");
    } catch (err) {
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        <View style={{ backgroundColor: colors.white, borderRadius: 20, padding: 22, marginTop: 24, ...shadow.card }}>
          <Text style={{ fontSize: 24, fontWeight: "800" }}>Verify your phone</Text>
          <Text style={{ color: colors.muted, marginTop: 6 }}>
            {phone || "Your number"}. Use 1234 for now. Your account is created only after this step succeeds.
          </Text>
          <TextInput
            value={code}
          ...
          <PressScale onPress={cancel} style={{ marginTop: 16, alignItems: "center" }}>
            <Text style={{ color: colors.muted, fontWeight: "700" }}>Cancel registration</Text>
          </PressScale>
        </View>
      </KeyboardScreen>
    </View>
